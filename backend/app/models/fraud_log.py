...
class FraudLog(Base):
    ...
    # ── Composite indexes ─────────────────────────────────────────────────────
    # Used by: fraud dashboard filtering by decision type and time
    __table_args__ = (
        Index("ix_fraud_logs_decision_created", "decision", "created_at"),
        # Used by: admin retrain query — fetch logs above a risk threshold
        Index("ix_fraud_logs_risk_score", "risk_score"),
    )

...
from app.core.dependencies import client_ip, get_db, require_admin, require_operator
...
def _actor(principal) -> str:
    return getattr(principal, "email", "break-glass")
...
def get_stats(db: Session = Depends(get_db), _=Depends(require_operator)):
...
def retrain_model(request: Request, db: Session = Depends(get_db),
    ...
    record(db, actor=_actor(admin), actor_role=getattr(admin, "role", "admin"),
...
def delete_user(user_id: int, request: Request, db: Session = Depends(get_db),
    ...
    if user_id == getattr(admin, "id", -999):
